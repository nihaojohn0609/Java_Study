...
public class Main {
    ...
    private static class StatsVisitor implements FileVisitor<Path> {
        ...
        public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
            ...
            if (parentMap != null) {    // It's possible that the current file's parent might be the original path => not part of my map!
...
//                throw exc;
            ...
            if(relativeLevel == 1) {
                folderSizes.forEach((key, value) -> {
                    ...
                    int level = key.getNameCount() - initialCount - 1;       // Don't indent the first level!
                    if(level < printLevel) {
